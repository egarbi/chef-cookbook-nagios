...
action :add do
  ...
  if new_resource.nagios_service and not data_bag("nagios_services").include?("nrpe_#{new_resource.command_name}")
    databag_item = Chef::DataBagItem.new
    databag_item.data_bag("nagios_services")
    databag_item.raw_data = {
      "id" => "nrpe_#{new_resource.command_name}",
      "hostgroup_name" => new_resource.recipe_name.gsub("client_","")
    }
    databag_item.save
  end
end
